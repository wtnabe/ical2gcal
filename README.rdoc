= ical2gcal

{<img src="https://travis-ci.org/wtnabe/ical2gcal.png" alt="Build Status" />}[https://travis-ci.org/wtnabe/ical2gcal]

sync ics(es) to google calendar

== Benefits

* Solve the PROBLEM that Android's calendar program cannot reference webcal !
* Complete sync calendar from .ics to google calendar ( also deleting events )

== Known Problems

Ical2gcal depends on gcalapi gem for google calendar access. Ical2gcal with 'gcalapi 0.1.2r' cannot be installed as standalone. ( I've failed to generate gcalapi gem package. ) Use with Bundler as below.

 gem 'gcalapi', :github => 'wtnabe/gcalapi'

and bundle install ( or bundle update ) .

Gcalapi 0.1.2r breaks compatibility with Ruby 1.8.x. So ical2gcal 0.4.2 does.

== Requirements

* PC :-)
* Ruby ( tested with REE 1.8.7 and CRuby 1.9.3)
* ri_cal      http://ri-cal.rubyforge.org/
* gcalapi     http://gcalapi.rubyforge.org/
* simple-rss  http://simple-rss.rubyforge.org/
* pit         http://coderepos.org/share/browser/lang/ruby/pit

== How to use

1. Install Ruby
2. `gem install ical2gcal`
3. Setup Your Google account info using Pit
4. Prepare calendar that you like to import to
5. type `ical2gcal --help`

== How to config pit

1. SET ENVIRONMENT VARIABLE *EDITOR*
2. type

 pit set google

ex)

    username: `google account'
    password: PASSWORD
    calendar:                       ( optional below )
      name: `calendar name above'

== Contributing to ical2gcal
 
* Check out the latest master to make sure the feature hasn't been implemented or the bug hasn't been fixed yet
* Check out the issue tracker to make sure someone already hasn't requested it and/or contributed it
* Fork the project
* Start a feature/bugfix branch
* Commit and push until you are happy with your contribution
* Make sure to add tests for it. This is important so I don't break it in a future version unintentionally.
* Please try not to mess with the Rakefile, version, or history. If you want to have your own version, or is otherwise necessary, that is fine, but please isolate to its own commit so I can cherry-pick around it.

== Copyright

Copyright (c) 2011 wtnabe. See LICENSE.txt for
further details.
